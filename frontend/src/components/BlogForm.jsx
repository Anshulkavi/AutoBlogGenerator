...
const BlogForm = ({ onGenerate, loading }) => {
  const [topic, setTopic] = useState('');
  ...
  return (
    ...
    >
      ...
      <input
        ...
        onChange={(e) => setTopic(e.target.value)}
        disabled={loading}
        className="p-3 border border-gray-300 rounded-lg shadow focus:outline-none focus:ring-2 focus:ring-blue-400 disabled:bg-gray-200"
      />
      <button
        type="submit"
        disabled={loading || !topic.trim()}
        className="bg-blue-600 text-white py-3 px-4 rounded-lg shadow hover:bg-blue-700 transition disabled:bg-gray-400 disabled:cursor-not-allowed"
      >
        {loading ? 'Generating...' : '🚀 Generate Blog'}
      </button>
    </form>
