...
import BlogForm from '../app/BlogForm'; // Use the new BlogForm
import BlogDisplay from '../app/BlogDisplay';
import { startBlogGeneration, getGenerationStatus } from '../../utils/api'; // Import the new functions
...
const AppPage = ({ onBackToHome }) => {
  ...
  const handleGenerate = async (topic) => {
    setLoading(true);
    setError('');
    setBlog(null); // Clear previous blog
    stopPolling(); // Make sure no old jobs are running

    try {
      const jobId = await startBlogGeneration(topic);
      ...
      pollingIntervalRef.current = setInterval(async () => {
        try {
          ...
          if (statusData.status === 'complete') {
            stopPolling();
            setLoading(false);
            setBlog(statusData.result); // Set the final blog data
      ...
      setLoading(false);
      setError('Failed to start the generation process.');
    }
  };
